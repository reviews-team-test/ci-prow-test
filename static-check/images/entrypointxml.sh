...
startCppCheck(){
    ...
    logdir="cppcheckReport/${REPO_NAME}-cppcheck-report.xml"
    cppcheck --enable=all --suppress='*:*3rdparty*' --library=./qt.cfg `cat cppcheck-files.txt` --xml 2> $logdir || true
    errNum=$(egrep "[[:space:]]+<error .*severity=\"error\"" $logdir | wc -l)
    if [ "$errNum" -gt "0" ];then
...
startTscancode(){
    ...
    logdir="tscancodeReport/${REPO_NAME}-tscancode-report.xml"
    tscancode --xml --enable=all `cat tscancode-files.txt` -q 2> $logdir || true
    errNum=$(egrep "[[:space:]]+<error .*severity=\"Serious\"|[[:space:]]+<error .*severity=\"Critical\"" $logdir | wc -l)
    if [ "$errNum" -gt "0" ];then
